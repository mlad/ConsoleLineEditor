...
internal sealed class LineEditorAnsiBuilder
{
    ...
    public void BuildRemoveCurrentLine(StringBuilder builder, LineEditorState state)
    {
        builder.Append("\u001b[?25l"); // Hide cursor

        // Render all lines
        for (var lineIndex = state.LineIndex + 1; lineIndex < state.LineCount; lineIndex++)
        {
            // Set cursor to beginning of line
            builder.Append("\u001b[1G");
        ...
        BuildLine(builder, state, state.Buffer);
